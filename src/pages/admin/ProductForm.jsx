...
export default function ProductForm() {
  ...
  const presetBrandId = searchParams.get("brand_id") || "";
  const returnTo = location.state?.from || searchParams.get("returnTo") || "/admin/products";
    ...
    const run = async () => {
      ...
      try {
        ...
        if (isEdit) {
          ...
          setBrandId(pickValidId(brandRows, presetBrandId));
          setSku("");
          setBarcode("");
          setPrice("0");
          setStock("0");
  ...
  }, [hydrateProduct, id, isEdit, loadMeta, loadProduct, presetBrandId, ui.and, ui.brand, ui.category, ui.inBackendBeforeSaving, ui.loadError, ui.missingBackendEntity, ui.noLongerExists, ui.savedEntityMissing]);
    ...
    try {
      if (isEdit) {
        ...
        if (pendingImages.length > 0) {
          ...
          setImages(Array.isArray(product.images) ? product.images : images);
        }
