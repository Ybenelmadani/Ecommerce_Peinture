...
export default function ProductCard({ p }) {
  ...
  const ui = pick({
    fr: {
      ...
      wishlistActive: "Retirer des favoris",
    },
    en: {
      ...
      wishlistActive: "Remove from wishlist",
    },
    ar: {
      ...
      wishlistActive: "إزالة من المفضلة",
    },
  });
    ...
    <div className="group flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-md">
      <div className="relative">
        ...
        <button
          type="button"
          onClick={() => toggle(p.id)}
          aria-label={inWishlist ? ui.wishlistActive : ui.wishlist}
          className={`absolute right-14 top-3 z-10 inline-flex h-9 w-9 items-center justify-center rounded-full border shadow-sm transition-colors ${
            inWishlist
              ? "border-rose-200 bg-rose-50 text-rose-600"
              : "border-slate-200 bg-white/95 text-slate-700 hover:bg-slate-900 hover:text-white"
          }`}
        >
