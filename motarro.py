...
BASE_URL = "https://www.motarro.com.au"
OUTPUT_FILE = "motarro.json"
TIMEOUT_SECONDS = 20
...
def extract_product_data(url, soup):
    ...
    if any(bad in path for bad in ["/news", "/blog", "/about", "/contact", "/privacy", "/terms"]):
        return None
...
def scrape_products(session, max_pages, checkpoint_path):
    ...
    if sitemap_urls:
        queue = deque(
            ...
            if urlparse(u).netloc.endswith("motarro.com.au") and not u.endswith(".xml")
        )
    else:
...
def main():
    parser = argparse.ArgumentParser(description="Scrape Motarro product pages and save JSON.")
