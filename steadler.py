...
BASE_URL = "https://www.staedtler.com/intl/en"
OUTPUT_FILE = "steadler.json"
TIMEOUT_SECONDS = 20
...
SITEMAP_URLS = [
    ...
    "https://www.staedtler.com/sitemap.xml",
    "https://www.staedtler.com/sitemap_index.xml",
    "https://www.staedtler.com/sitemap.xml.gz",
]
...
def extract_product_data(url, soup):
    ...
    for link in soup.find_all("a", href=lambda x: x and x.lower().endswith(".pdf")):
        ...
        if pdf_url:
            pdf_links.append(urljoin(url, pdf_url))
...
def scrape_products(session, max_pages, checkpoint_path):
    ...
    if sitemap_urls:
        queue = deque(u for u in sitemap_urls if urlparse(u).netloc.endswith("staedtler.com"))
    else:
...
def main():
    parser = argparse.ArgumentParser(description="Scrape STAEDTLER product pages and save JSON.")
